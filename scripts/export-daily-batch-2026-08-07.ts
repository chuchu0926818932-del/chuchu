...
import { dailyTopics20260807 } from "../app/daily-topics-2026-08-07";
...
const topicMarkdown = (topic: (typeof dailyTopics20260807)[number], index: number) => `## ${index + 1}. ${topic.title}
...
${dailyTopics20260807.map(topicMarkdown).join("\n\n---\n\n")}
`;

await mkdir(outputDir, { recursive: true });
await writeFile(outputPath, markdown, "utf8");
await writeFile(dataPath, JSON.stringify(dailyTopics20260807, null, 2), "utf8");
console.log(JSON.stringify({ outputPath, dataPath, count: dailyTopics20260807.length }));
