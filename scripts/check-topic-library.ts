...
const dailyTopics = topics.filter((topic) => topic.id.startsWith("D20260901-"));
...
assert(topics.length === 890, `Expected 890 topics total, received ${topics.length}.`);
...
for (const field of ["explain", "singleCta"] as const) {
  const priorValues = new Set(topics.filter((topic) => !topic.id.startsWith("D20260901-")).map((topic) => normalize(topic[field])));
