...
const dailyTopics = topics.filter((topic) => topic.id.startsWith("D20260922-"));
...
assert(topics.length === 1070, `Expected 1070 topics total, received ${topics.length}.`);
...
for (const field of ["explain", "singleCta"] as const) {
  const priorValues = new Set(topics.filter((topic) => !topic.id.startsWith("D20260922-")).map((topic) => normalize(topic[field])));
