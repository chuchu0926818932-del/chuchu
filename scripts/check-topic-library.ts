...
const dailyTopics = topics.filter((topic) => topic.id.startsWith("D20260919-"));
...
assert(topics.length === 1040, `Expected 1040 topics total, received ${topics.length}.`);
...
for (const field of ["explain", "singleCta"] as const) {
  const priorValues = new Set(topics.filter((topic) => !topic.id.startsWith("D20260919-")).map((topic) => normalize(topic[field])));
...
for (const topic of [...priorDailyThirtyTopicBatch, ...priorNewestDailyTopics, ...priorFinalDailyTopics, ...topics.filter((topic) => topic.id.startsWith("D20260918-")), ...dailyTopics, ...foundationalTopics]) {
...
assert(categorizedTopicCounts.get("女性成長") === 77, "女性成長 must have 77 categorized topics.");
assert(categorizedTopicCounts.get("金錢價值觀") === 77, "金錢價值觀 must have 77 categorized topics.");
assert(categorizedTopicCounts.get("親子關係") === 76, "親子關係 must have 76 categorized topics.");
