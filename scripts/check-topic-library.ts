...
const dailyTopics = topics.filter((topic) => topic.id.startsWith("D20260918-"));
...
assert(topics.length === 1010, `Expected 1010 topics total, received ${topics.length}.`);
...
for (const field of ["explain", "singleCta"] as const) {
  const priorValues = new Set(topics.filter((topic) => !topic.id.startsWith("D20260918-")).map((topic) => normalize(topic[field])));
