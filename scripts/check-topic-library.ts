...
for (const topic of [...priorDailyThirtyTopicBatch, ...priorNewestDailyTopics, ...priorFinalDailyTopics, ...topics.filter((topic) => topic.id.startsWith("D20260918-")), ...topics.filter((topic) => topic.id.startsWith("D20260922-")), ...dailyTopics, ...foundationalTopics]) {
...
assert(categorizedTopicCounts.get("女性成長") === 87, "女性成長 must have 87 categorized topics.");
assert(categorizedTopicCounts.get("金錢價值觀") === 87, "金錢價值觀 must have 87 categorized topics.");
assert(categorizedTopicCounts.get("親子關係") === 86, "親子關係 must have 86 categorized topics.");
