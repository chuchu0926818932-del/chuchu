...
import { dailyTopics20260816 } from "../app/daily-topics-2026-08-16";
...
const topicMarkdown = (topic: (typeof dailyTopics20260816)[number], index: number) => `## ${index + 1}. ${topic.title}
...
await writeFile(outputPath, `# ${date} 短影音企劃 30 條\n\n女性成長（職涯／事業）、金錢價值觀、親子關係各 10 條。既有退役、每日與基礎題庫只作嚴格去重參考，沒有改寫、拼貼或延伸。\n\n${dailyTopics20260816.map(topicMarkdown).join("\n\n---\n\n")}\n`, "utf8");
await writeFile(dataPath, JSON.stringify(dailyTopics20260816, null, 2), "utf8");
console.log(JSON.stringify({ outputPath, dataPath, count: dailyTopics20260816.length }));
