...
import { dailyTopics20260818 } from "../app/daily-topics-2026-08-18";
...
const topicMarkdown = (topic: (typeof dailyTopics20260818)[number], index: number) => `## ${index + 1}. ${topic.title}
...
await writeFile(outputPath, `# ${date} 短影音企劃 30 條\n\n女性成長（職涯／事業發展）、金錢價值觀、親子關係各 10 條；所有企劃採日常痛點、承接限制與情緒、白話拆解、低門檻選擇、轉念收尾與單一關鍵字 CTA。\n\n${dailyTopics20260818.map(topicMarkdown).join("\n\n---\n\n")}\n`, "utf8");
await writeFile(dataPath, JSON.stringify(dailyTopics20260818, null, 2), "utf8");
console.log(JSON.stringify({ outputPath, dataPath, count: dailyTopics20260818.length }));
